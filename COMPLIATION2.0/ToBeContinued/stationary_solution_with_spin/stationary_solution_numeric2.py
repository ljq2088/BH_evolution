...
import stationary_solution_symbol2 as symbol
...
def horizon_condition(h,f,chi,xi,A,B):
    ...
    return Dx_odd.dot(xi_h+px_h/eAchi_h*coshB_h)+(f_h-px_h**2/eAchi_h*coshB_h)/h
...
eta3=np.arange(0,0.2,0.001)

f_tilde0_number=len(f_tilde0)
eta3_number=len(eta3)
stationary_solution=np.zeros((eta3_number,f_tilde0_number,7,N*m))
...
for i in range(0,f_tilde0_number):
    ...
    f_tilde_Sch=np.kron(initial_static_solution[i,0],np.ones(m))
    chi_Sch=np.kron(initial_static_solution[i,1],np.ones(m))
    phi_tilde_Sch=np.kron(initial_static_solution[i,2],np.ones(m))
    first_iteration_initial_value=np.array([chi_Sch,xi_tilde_Sch,eta_tilde_Sch,f_tilde_Sch,A_Sch,B_Sch,phi_tilde_Sch])

    '''
    constraint_fields=np.load('./final_state/constraint_fields.npy')
    evolution_fields=np.load('./final_state/evolution_fields.npy',allow_pickle=True)
    h=np.load('./final_state/h.npy')
    ...
    chi_tilde=(chi-2**2/8*z0[:,None]**2)/z0[:,None]**3
    chi_tilde[-1]=2*Dz2[-1].dot(phi)/2/3
    xi_tilde=xi/z0[:,None]**3*np.sin(x0)
    xi_tilde[-1]=xi3*np.sin(x0)
    f_tilde=(f_tilde-(2**2/8+1))/z0[:,None]
    f_tilde[-1]=f3
    A_tilde=A/z0[:,None]**3
    A_tilde[-1]=Dz[-1].dot(Dz2).dot(A)/6
    phi_tilde=(phi-2*z0[:,None])/z0[:,None]**2
    phi_tilde[-1]=Dz2[-1].dot(phi)/2

    first_iteration_initial_value=np.array([chi_tilde.reshape(N*m),xi_tilde.reshape(N*m),eta_tilde_Sch,f_tilde.reshape(N*m),A_tilde.reshape(N*m),B_Sch,phi_tilde.reshape(N*m)])
    '''
    ...
    for j in range(0,eta3_number):
        ...
        --------------------------------------------
        """
        stationary_solution[i,-1]=first_iteration_initial_value.copy()
        time_start=time.time()
        iteration_initial_value=stationary_solution[i,j-1].copy()
        boundary_condition=[(1,1,-1,1,0),(6,6,-1,1,source),(3,symbol.boundary_condition_f_variation_function,-1,4,symbol.boundary_condition_f_function),(5,symbol.boundary_condition_B_variation_function,-1,4,symbol.boundary_condition_B_function),(3,3,0,5,f_tilde_Sch[0]),(5,2,-1,5,eta3[j])]
        stationary_solution[i,j]=newton_iteration(symbol.variation_function,symbol.equations_function,arg_parameter,[z,x],[N,m],iteration_initial_value,boundary_condition,differentiation_matrixes,1e-6,True)
        error[i,j]=symbol.error_function(*arg_parameter,*[z,x],*numeric_arg_field(stationary_solution[i,j],differentiation_matrixes))
        print('max(error):',np.abs(error[i]).max())

        chi,xi_tilde,eta_tilde,f_tilde,A_tilde,B_tilde,phi_tilde=stationary_solution[i,j].reshape(7,N,m)
        ...
        h[i,j]=scipy.optimize.root(horizon_condition,np.ones(m),(f,chi,xi,A,B)).x
        temperature[i,j]=get_temperature(h[i,j],chi,xi,eta,f,A,B)
        entropy[i,j]=(2*np.pi/h[i,j]**2*np.sin(x0)).sum(axis=-1)*np.pi/m/2*4*np.pi

        # energy_density[i]=-Dz[-1].dot(stationary_solution[i,0])+source*Dz[-1].dot(stationary_solution[i,2])/6
        print(f'T={temperature[i,j]}, S={entropy[i,j]}, epsilon={energy_density[i,j]}')
