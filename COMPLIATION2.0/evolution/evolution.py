...
phi1=2
index_temp=118
file_location_solution=f'../static_solution/BS/EOS/source={phi1}/z0={z0[0]}/{N=}'
static_solution=np.load(file_location_solution+'/1.data/1.static_solution.npy')[index_temp]

A[0]=np.zeros((N,m))
phi0=0.001
# phi[0]=z*static_solution[2,:,None]+phi0*z**2*np.cos(2*np.pi/length*x0)
# phi[0]=z*static_solution[2,:,None]+phi0*z**2*(3*np.cos(2*np.pi/length*x0)**2-1)/2
phi[0]=z*static_solution[2,:,None]+phi0*z**2*np.exp(-10*np.sin(np.pi/length*x0)**2)
xi3[0]=np.zeros(m)
f3[0]=D1[-1].dot(static_solution[0,:,None])
evolution_fields[0]=np.array([A[0],phi[0],xi3[0],f3[0]],dtype=object)

plt.plot(x0,(phi[0]-z*static_solution[2,:,None])[0])
plt.show()
plt.close()
...
file_location=f'./data/source={phi1}/z0={z0[0]}_n_temp={index_temp}_gaussian/{N=}_{m=}_{dt=}_{t_last}_{phi0=}'
...
plt.plot(time_array,f3[:,15])
plt.plot(time_array,-2+phi0*np.cos(2*np.pi/length*x0[15])*np.cos(time_array))
# plt.plot(time_array,5.8878*xi3[:,15])
# plt.plot(time_array,phi0*np.cos(2*np.pi/length*x0[15])*np.sin(time_array))
plt.xlabel('$t$')
plt.ylabel('$f_3$')
